...
def report_progress(typed, source, user_id, upload):
    ...
    for i in range(len(typed)):
        if typed[i] == source[i]:
            ...
            break
    progress = progess / len(source)
    upload({'id': user_id, 'progress': progess / len(source)})
    return progess / len(source)
    # END PROBLEM 8
...
def fastest_words(words_and_times):
    ...
    for word_idx in word_indices:
        fastest_player = min(player_indices, key=lambda i: times[i][word_idx])
        fastest_words[fastest_player].append(words[word_idx])
